...
export function Header({ onSync, isPipelineActive }: HeaderProps) {
  return (
    <header className="bg-white border-b border-slate-200 px-6 py-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between sticky top-0 z-10 shadow-sm" aria-label="Cabeçalho do painel">
      ...
      <div className="flex items-center gap-3 md:flex-shrink-0">
        ...
        <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold border transition-colors ${
          ...
          <span className="relative flex h-2 w-2">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full opacity-75" style={{ backgroundColor: isPipelineActive ? '#10b981' : '#94a3b8' }} />
            <span className="relative inline-flex rounded-full h-2 w-2" style={{ backgroundColor: isPipelineActive ? '#059669' : '#64748b' }} />
          </span>

...
function App() {
  ...
  const emailPipelineItems = [
    { title: 'Principal', queueName: 'q.email', icon: <Activity className="w-3 h-3" />, activeColor: '#3b82f6', activeBg: '#eff6ff' },
    { title: 'Tentativa 1', queueName: 'q.email.retry.0', icon: <Clock className="w-3 h-3" />, activeColor: '#f59e0b', activeBg: '#fffbeb' },
    { title: 'Tentativa 2', queueName: 'q.email.retry.1', icon: <Clock className="w-3 h-3" />, activeColor: '#f97316', activeBg: '#fff7ed' },
    { title: 'Tentativa 3', queueName: 'q.email.retry.2', icon: <Clock className="w-3 h-3" />, activeColor: '#ef4444', activeBg: '#fef2f2' },
  ];

  const smsPipelineItems = [
    { title: 'Principal', queueName: 'q.sms', icon: <Activity className="w-3 h-3" />, activeColor: '#10b981', activeBg: '#f0fdf4' },
    { title: 'Tentativa 1', queueName: 'q.sms.retry.0', icon: <Clock className="w-3 h-3" />, activeColor: '#f59e0b', activeBg: '#fffbeb' },
    { title: 'Tentativa 2', queueName: 'q.sms.retry.1', icon: <Clock className="w-3 h-3" />, activeColor: '#f97316', activeBg: '#fff7ed' },
    { title: 'Tentativa 3', queueName: 'q.sms.retry.2', icon: <Clock className="w-3 h-3" />, activeColor: '#ef4444', activeBg: '#fef2f2' },
  ];

  return (
    <main className="min-h-screen bg-slate-50 text-slate-800 font-sans antialiased" style={{ fontFamily: "'Inter', sans-serif" }}>
      <Header
        onSync={() => { fetchDlq(); addLog('Sincronização manual realizada.', 'info'); }}

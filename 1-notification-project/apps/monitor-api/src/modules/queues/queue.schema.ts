...
export const QueueSchema = z.object({
  ...
  messages: z.number().int().describe('Total number of messages currently in the queue, including unacknowledged ones.'),
  ...
  headTimestamp: z.number().nullable().describe('Unix timestamp (ms) of the oldest message in the queue. Example: 1716400000000'),
}).describe('RabbitMQ queue');

export const GetQueuesResponseSchema = z.object({
  serverTime: z.number().int().describe('Unix timestamp (ms) of when this response was generated. Example: 1716400012345'),
  queues: z.array(QueueSchema).describe('List of all queues visible to the Monitor API.'),
}).describe('OK');

export const DlqMessagePayloadSchema = z.object({
...
export const DlqMessageSchema = z.object({
  payload: z.union([z.string(), DlqMessagePayloadSchema]).describe('The body of the dead-lettered message. Can be a raw string or a parsed notification object.'),
  properties: z.object({
    delivery_mode: z.number().int().optional().describe('RabbitMQ delivery mode. 2 = persistent. Example: 2'),
    headers: z.record(z.string(), z.unknown()).optional().describe('AMQP headers attached to the message, including dead-letter metadata.'),
  }).optional().describe('AMQP properties of the message.'),
  redelivered: z.boolean().describe('Whether this message has been redelivered at least once.'),
  routing_key: z.string().describe('The routing key used when the message was originally published. Example: q.notifications.dlq'),
}).describe('A single message peeked from the Dead Letter Queue.');

export const DeleteDlqResponseSchema = z.object({
...
export const GetQueuesRouteSchema = {
  ...
  description: 'Returns real-time statistics for all RabbitMQ queues visible to the management API, including message counts and TTL settings.',
  response: {
...
export const StreamQueuesRouteSchema = {
  ...
  description: 'Opens a Server-Sent Events (SSE) stream that pushes queue statistics every 500ms. Connect via `EventSource` in the browser. The response body is a continuous `text/event-stream` and is not a standard JSON response.',
  response: {},
};
...
export const GetDlqRouteSchema = {
  ...
  description: 'Peeks up to 20 messages from the Dead Letter Queue (`q.notifications.dlq`) without removing them. Messages are requeued after inspection.',
  response: {
...
export const DeleteDlqRouteSchema = {
  tags: ['Dead Letter Queue'],
  summary: 'Purge DLQ',
  description: 'Permanently deletes **all messages** from the Dead Letter Queue (`q.notifications.dlq`). This action is irreversible.',
  response: {
